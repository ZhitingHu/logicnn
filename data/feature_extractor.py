...
def extract_but(revs):
    but_fea = []
    but_ind = []
    but_fea_cnt, though_fea_cnt = 0, 0
    for rev in revs:
        text = rev["text"]
        if ' but ' in text:
            but_ind.append(1)
...
if __name__=="__main__":
    ...
    # extract BUT-rule feature
    fea = extract_but(revs)
    # dump
    outfile_name = "%s.fea.p" % data_file
    cPickle.dump(fea, open(outfile_name, "wb"))
    print "feature dumped to %s!" % outfile_name
